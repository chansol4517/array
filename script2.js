...
/*
참조형 자료는 변수에 값을 옮겨 닮으면 얕은 복사 처리됨(Shallow Copy)
*/
//자바스크립트에서 let, const형석으로 만든 변수의 메모리 공간은 callstack에 생성됨
//실제 callstack의 arr라는 변수에는 배열이 위치하고 있는 힘메모리상의 위치값(참조값) 이 담겨있음
const arr = [1, 2, 3];
//arr변수 자체에 메모리힙상의 위치값만 들어가 있기 때문에
//아래 경우 해당 힙메모리상의 값을 바꾼것 뿐 callstack에 위치해 있는 arr에 담겨있는 참조값이 바뀐것이 아님
